/*
 *--------------------------------------
 * Program Name: XENON CLI
 * Author: ALVAJOY
 * License: MIT
 * Description: SHELL INTERFACE OF XENON (v1.0)
 *--------------------------------------
 */
...
void main(void)
{
	...
	/* choose users (1 means user picked, 0 means no users (make a new user), 2 forced exit)*/
	...
	switch (init_users()) {
		...
		case 1:
			// goto password if any.
			switch(CheckforPassword(terminal_info.userpos)){
				case 0:
					goto terminal_input;
				break;
