...
static void core_InitData(void)
{
	/* Text Print Position */
	...
	core_data.folder_pos = 0;
}

static void core_SetupAutoFill(void)
{
	for (int i = 0; i < oxy_file_system.numfiles; i++)
	{
		oxy_StringInput_AddToAutofill(oxy_file[i].name);
	}

	for (int i = 0; i < oxy_file_system.numfolders; i++)
	{
		oxy_StringInput_AddToAutofill(oxy_folder[i].name);
	}
}
...
void core_CheckCommand(char *str)
{
	...
	if (oxy_stringinput.forced_exit)
		return;
		...
		if (!strncasecmp(str, "EXEC", 4))
		{
			...
			for (int i = 0; i < oxy_file_system.numfiles; i++)
			{
				if (!strcmp(oxy_file[i].name, token))
				{
					...
					{
						if (oxy_GetFileType(oxy_file[i].type) != OS_TYPE_APPVAR)
						{
							core_RunProgram(i);
						}
						else
			...
			for (int i = 0; i < oxy_file_system.numfiles; i++)
			{
				if (!strcmp(oxy_file[i].name, token[0]))
				{
					if (oxy_file[i].location == core_data.folder_pos)
					{
						...
						if (oxy_GetFileType(oxy_file[i].type) == OS_TYPE_PRGM)
						{
